...
class TestPassportInfoDao {

    @Mock SessionFactory sessionFactory;
    @Mock Session session;
    @Mock Query<?> query;

    @InjectMocks PassportInfoDao passportInfoDao;

    @Test
    void findAllShouldReturnListWhenSupported() throws Exception {
        Method method = getMethodOrNull(PassportInfoDao.class, "findAll");
        ...
        Object result = method.invoke(passportInfoDao);
        assertNotNull(result);
    }

    @Test
    void findByIdShouldReturnValueWhenSupported() throws Exception {
        Method method = getMethodOrNull(PassportInfoDao.class, "findById", Long.class);
        if (method == null) method = getMethodOrNull(PassportInfoDao.class, "findById", long.class);
        Assumptions.assumeTrue(method != null);

        when(sessionFactory.openSession()).thenReturn(session);
        Object result = method.invoke(passportInfoDao, 1L);
        assertNotNull(result);
    }
