...
class TestVisaTypeDao {

    @Mock SessionFactory sessionFactory;
    @Mock Session session;
    @Mock Query<?> query;

    @InjectMocks VisaTypeDao visaTypeDao;

    @Test
    void findAllShouldReturnListWhenSupported() throws Exception {
        Method method = getMethodOrNull(VisaTypeDao.class, "findAll");
        ...
        Object result = method.invoke(visaTypeDao);
        assertNotNull(result);
    }

    @Test
    void findByIdShouldReturnValueWhenSupported() throws Exception {
        Method method = getMethodOrNull(VisaTypeDao.class, "findById", Long.class);
        if (method == null) method = getMethodOrNull(VisaTypeDao.class, "findById", long.class);
        Assumptions.assumeTrue(method != null);

        when(sessionFactory.openSession()).thenReturn(session);
        Object result = method.invoke(visaTypeDao, 1L);
        assertNotNull(result);
    }
