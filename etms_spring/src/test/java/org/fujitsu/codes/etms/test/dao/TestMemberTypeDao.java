...
class TestMemberTypeDao {

    @Mock SessionFactory sessionFactory;
    @Mock Session session;
    @Mock Query<?> query;

    @InjectMocks MemberTypeDao memberTypeDao;

    @Test
    void findAllShouldReturnListWhenSupported() throws Exception {
        Method method = getMethodOrNull(MemberTypeDao.class, "findAll");
        ...
        Object result = method.invoke(memberTypeDao);
        assertNotNull(result);
    }

    @Test
    void findByIdShouldReturnValueWhenSupported() throws Exception {
        Method method = getMethodOrNull(MemberTypeDao.class, "findById", Long.class);
        if (method == null) method = getMethodOrNull(MemberTypeDao.class, "findById", long.class);
        Assumptions.assumeTrue(method != null);

        when(sessionFactory.openSession()).thenReturn(session);
        Object result = method.invoke(memberTypeDao, 1L);
        assertNotNull(result);
    }
