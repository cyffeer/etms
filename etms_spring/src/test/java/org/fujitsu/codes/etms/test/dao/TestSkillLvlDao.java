...
class TestSkillLvlDao {

    @Mock SessionFactory sessionFactory;
    @Mock Session session;
    @Mock Query<?> query;

    @InjectMocks SkillLvlDao skillLvlDao;

    @Test
    void findAllShouldReturnListWhenSupported() throws Exception {
        Method method = getMethodOrNull(SkillLvlDao.class, "findAll");
        ...
        Object result = method.invoke(skillLvlDao);
        assertNotNull(result);
    }

    @Test
    void findByIdShouldReturnValueWhenSupported() throws Exception {
        Method method = getMethodOrNull(SkillLvlDao.class, "findById", Long.class);
        if (method == null) method = getMethodOrNull(SkillLvlDao.class, "findById", long.class);
        Assumptions.assumeTrue(method != null);

        when(sessionFactory.openSession()).thenReturn(session);
        Object result = method.invoke(skillLvlDao, 1L);
        assertNotNull(result);
    }
