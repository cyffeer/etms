...
class TestVendorTypeDao {

    @Mock SessionFactory sessionFactory;
    @Mock Session session;
    @Mock Query<?> query;

    @InjectMocks VendorTypeDao vendorTypeDao;

    @Test
    void findAllShouldReturnListWhenSupported() throws Exception {
        Method method = getMethodOrNull(VendorTypeDao.class, "findAll");
        ...
        Object result = method.invoke(vendorTypeDao);
        assertNotNull(result);
    }

    @Test
    void findByIdShouldReturnValueWhenSupported() throws Exception {
        Method method = getMethodOrNull(VendorTypeDao.class, "findById", Long.class);
        if (method == null) method = getMethodOrNull(VendorTypeDao.class, "findById", long.class);
        Assumptions.assumeTrue(method != null);

        when(sessionFactory.openSession()).thenReturn(session);
        Object result = method.invoke(vendorTypeDao, 1L);
        assertNotNull(result);
    }
