...
class TestNpTestHistDao {

    @Mock SessionFactory sessionFactory;
    @Mock Session session;
    @Mock Query<?> query;

    @InjectMocks NpTestHistDao npTestHistDao;

    @Test
    void findAllShouldReturnListWhenSupported() throws Exception {
        Method method = getMethodOrNull(NpTestHistDao.class, "findAll");
        ...
        Object result = method.invoke(npTestHistDao);
        assertNotNull(result);
    }

    @Test
    void findByIdShouldReturnValueWhenSupported() throws Exception {
        Method method = getMethodOrNull(NpTestHistDao.class, "findById", Long.class);
        if (method == null) method = getMethodOrNull(NpTestHistDao.class, "findById", long.class);
        Assumptions.assumeTrue(method != null);

        when(sessionFactory.openSession()).thenReturn(session);
        Object result = method.invoke(npTestHistDao, 1L);
        assertNotNull(result);
    }
