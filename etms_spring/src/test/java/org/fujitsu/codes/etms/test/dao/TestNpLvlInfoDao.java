...
class TestNpLvlInfoDao {

    @Mock SessionFactory sessionFactory;
    @Mock Session session;
    @Mock Query<?> query;

    @InjectMocks NpLvlInfoDao npLvlInfoDao;

    @Test
    void findAllShouldReturnListWhenSupported() throws Exception {
        Method method = getMethodOrNull(NpLvlInfoDao.class, "findAll");
        ...
        Object result = method.invoke(npLvlInfoDao);
        assertNotNull(result);
    }

    @Test
    void findByIdShouldReturnValueWhenSupported() throws Exception {
        Method method = getMethodOrNull(NpLvlInfoDao.class, "findById", Long.class);
        if (method == null) method = getMethodOrNull(NpLvlInfoDao.class, "findById", long.class);
        Assumptions.assumeTrue(method != null);

        when(sessionFactory.openSession()).thenReturn(session);
        Object result = method.invoke(npLvlInfoDao, 1L);
        assertNotNull(result);
    }
