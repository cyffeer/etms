...
class TestVendorInfoDao {

    @Mock SessionFactory sessionFactory;
    @Mock Session session;
    @Mock Query<?> query;

    @InjectMocks VendorInfoDao vendorInfoDao;

    @Test
    void findAllShouldReturnListWhenSupported() throws Exception {
        Method method = getMethodOrNull(VendorInfoDao.class, "findAll");
        ...
        Object result = method.invoke(vendorInfoDao);
        assertNotNull(result);
    }

    @Test
    void findByIdShouldReturnValueWhenSupported() throws Exception {
        Method method = getMethodOrNull(VendorInfoDao.class, "findById", Long.class);
        if (method == null) method = getMethodOrNull(VendorInfoDao.class, "findById", long.class);
        Assumptions.assumeTrue(method != null);

        when(sessionFactory.openSession()).thenReturn(session);
        Object result = method.invoke(vendorInfoDao, 1L);
        assertNotNull(result);
    }
